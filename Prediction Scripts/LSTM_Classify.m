...
save(fullfile(outputFolder, 'trained_model_threeupperthighemg.mat'), 'net');
...
% Test on a shortened sequence
if ~isempty(XTest)
    testSampleIdx = randi(numel(XTest));
    originalData = XTest{testSampleIdx};
    originalLabel = YTest(testSampleIdx);
    
    shortenedLength = min(10, size(originalData, 2));
    shortenedData = originalData(:, 1:shortenedLength);
    
    shortenedPrediction = classify(net, {shortenedData});
    
    fprintf('Original sequence length: %d\n', size(originalData, 2));
    fprintf('Shortened sequence length: %d\n', size(shortenedData, 2));
    fprintf('True label: %s\n', char(originalLabel));
    fprintf('Prediction on shortened sequence: %s\n', char(shortenedPrediction{1}(1)));
    
    correctPrediction = shortenedPrediction{1}(1) == originalLabel;
    fprintf('Correct prediction: %s\n', string(correctPrediction));
end
